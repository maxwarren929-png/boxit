...
function createContextMenus() {
  chrome.contextMenus.removeAll(() => {
    chrome.contextMenus.create({ id: 'boxit-save-image', title: 'Save image to BoxIt', contexts: ['image'] });
    chrome.contextMenus.create({ id: 'boxit-save-link', title: 'Save linked file to BoxIt', contexts: ['link'] });
  });
}

async function clearCaptureDefaultIfNeeded(boxId) {
  const stored = await chrome.storage.local.get(CAPTURE_BOX_KEY);
  if (stored[CAPTURE_BOX_KEY] === boxId) await chrome.storage.local.remove(CAPTURE_BOX_KEY);
}
...
async function deleteLifecycleBox(boxId, reason = 'expired') {
  ...
  try {
    await chrome.runtime.sendMessage({ type: 'BOXIT_BOX_REMOVED', boxId, boxName: box.name, reason });
  } catch {}
  return box;
}
...
async function scheduleAllExpirations() {
  ...
  await Promise.all(
    alarms.filter(alarm => alarm.name.startsWith(EXPIRY_ALARM_PREFIX)).map(alarm => chrome.alarms.clear(alarm.name))
  );
...
async function startupMaintenance({ clearSession = false } = {}) {
  if (clearSession) {
...
chrome.runtime.onInstalled.addListener(() => {
  createContextMenus();
  startupMaintenance().catch(console.error);
});

chrome.runtime.onStartup.addListener(() => {
  startupMaintenance({ clearSession: true }).catch(console.error);
});
...
function nameFromUrl(url, type, headers) {
  ...
  const extension = extensionForType(type);
  return `capture-${new Date().toISOString().replace(/[:.]/g, '-')}${extension ? `.${extension}` : ''}`;
}
...
async function fetchFromPage(tabId, url) {
  const prepared = await sendPageMessage(tabId, { type: 'BOXIT_RESOURCE_PREPARE_V3', url });
  if (!prepared?.ok || !prepared.transferId) throw new Error(prepared?.error || 'The page could not provide this resource.');
  const chunks = [];
  try {
    for (let offset = 0; offset < prepared.size; offset += RESOURCE_CHUNK_BYTES) {
      const response = await sendPageMessage(tabId, {
        type: 'BOXIT_RESOURCE_READ_V3', transferId: prepared.transferId, offset,
        length: Math.min(RESOURCE_CHUNK_BYTES, prepared.size - offset)
      });
      if (!response?.ok) throw new Error(response?.error || 'The page-resource transfer failed.');
      chunks.push(new Uint8Array(response.bytes || []));
    }
  } finally {
    sendPageMessage(tabId, { type: 'BOXIT_RESOURCE_RELEASE_V3', transferId: prepared.transferId }).catch(() => {});
  }
  return { blob: new Blob(chunks, { type: prepared.type || 'application/octet-stream' }), url: prepared.url || url, headers: null };
}
...
async function fetchRemoteResource(url, pageTabId = null) {
  ...
  try {
    ...
    if (length > MAX_REMOTE_CAPTURE_BYTES) throw new Error('This file is larger than BoxIt can capture right now.');
    const blob = await response.blob();
    if (blob.size > MAX_REMOTE_CAPTURE_BYTES) throw new Error('This file is larger than BoxIt can capture right now.');
    return { blob, url: response.url || url, headers: response.headers };
  } catch (error) {
    directError = error;
  }
  if (pageTabId) {
    try { return await fetchFromPage(pageTabId, url); } catch {}
  }
  throw directError || new Error('BoxIt could not fetch this file.');
}

async function publishCapture(record, box) {
  const event = { fileId: record.id, fileName: record.name, boxId: box.id, boxName: box.name, capturedAt: Date.now() };
  await chrome.storage.local.set({ [LAST_CAPTURE_KEY]: event });
  await chrome.action.setBadgeText({ text: '1' });
  try { await chrome.runtime.sendMessage({ type: 'BOXIT_CAPTURE_SAVED', capture: event }); } catch {}
  return event;
}
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  try {
    if (info.menuItemId === 'boxit-save-image') await saveRemoteUrl(info.srcUrl, 'context-image', null, tab?.id || null);
    else if (info.menuItemId === 'boxit-save-link') await saveRemoteUrl(info.linkUrl, 'context-link', null, tab?.id || null);
  } catch (error) {
    const message = error?.message || 'BoxIt could not save that item.';
    await chrome.storage.local.set({ [LAST_CAPTURE_KEY]: { error: message, capturedAt: Date.now() } });
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message?.type === 'BOXIT_LIFECYCLE_CHANGED') {
    getBox(message.boxId).then(box => box ? scheduleBoxExpiry(box) : null)
  ...
  if (message?.type === 'BOXIT_COMPLETE_ONE_SHOT') {
    sendResponse({ ok: true, deleted: false, pending: true });
    return;
  }
  if (message?.type === 'BOXIT_CONFIRM_ONE_SHOT_DELETE') {
    getBox(message.boxId).then(async box => {
